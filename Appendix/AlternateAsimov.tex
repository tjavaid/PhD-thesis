...
Additional observables include $\Phi$, $\Phi_{1}$, $\mathrm{m}(\mathrm{Z}_{1})$, $\mathrm{m}(\mathrm{Z}_{2})$, $|\cos \theta^{*}|$, $|\cos \theta_{1}|$, $|\cos \theta_{2}|$, number of jets (within $|\eta|<4.7$) and $\pt$ of leading jet (within $|\eta|<4.7$) which are studied in $\Hllll$ using full Run 2 toy data and compared with the theoretical predictions.
