...
%\end{center}
%\end{figure} 
\subsubsection{Cathode Strip Chambers  (CSC)} \label{csc_cms}
...
In a chamber layer, Cathode Front-End Boards (CFEBs) \footnote{Electronics or readout system of ME/1/1 chambers were upgraded during long shutdown 1 (LS1), hence differs from other chambers.} receive and then process the signal from strips. However, from anode wires, signal is read by Anode Front-End Boards (AFEBs) and processed by Anode Local Charged Track (ALCT) board. These boards then communicate with the Trigger Mother Board (TMB) located in the vicinity of the endcap disks. Data is then transmitted to Data Mother Board (DMB) which sends it out Front End Driver (FED) crates whose front end electronics has interface with CMS data acquisition system. Overview of the chamber electronics and communication mechanism is elaborated in Figure. \ref{Fig:csc_chamber_electronics}.
