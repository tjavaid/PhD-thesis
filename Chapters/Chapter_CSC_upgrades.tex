...
%\chapter{The Cathode Strip Chambers Upgrades after Run 2 operations} % Main chapter title
%
%\label{Chapter_CSC_upgrades} % For referencing the chapter elsewhere, use \ref{Chapter2_CMS} 
%----------------------------------------------------------------------------------------
%==========================================================================================
%To extend the sensitivity for new physics searches, a major upgrade of the LHC has been decided and is being prepared, the High Luminosity LHC (HL-LHC). The integrated luminosity will increase tenfold with respect to the original design values, as shown in Table,  which summarizes some key parameters at LHC and HL-LHC for proton-proton running [references].
%\section{Overview} \label{overview_upgrade}
An overview of the CSCs of CMS muon system is described in Section \ref{csc_cms}. CSC system is located in the endcap region of CMS and provides precise measurements of the position of hits along the trajectories of muons in the forward regions. It has successfully delivered during Run 1 and Run 2 operations of LHC helping the physics world to make revolutionary advancements e.g. discovery of Higgs as described in Chapter \ref{thesis_introduction}. 
Figure \ref{cms_higgs_signature} shows the event display of (probably) Higgs decaying to four muons (red lines) detected in muon system of CMS.
\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{Figures/cms_higgs_signature.png}
\caption{Event display for four muons in final state in muon system of CMS. 
        }
\label{cms_higgs_signature}
\end{center}
\end{figure}
\par LHC has now aimed to operate at even higher luminosity which will increase the sensitivity for the physics analyses particularly for the new physics phenomena. 
%The projected evolution of instantaneous and integrated luminosity with time is shown in Fig. 1.3, for the design scheme and also for an ultimate scenario, which can possibly be reached by exploiting all LHC margins. 
%The center-of-mass energy for proton-proton collisions is expected to be raised from the current 13 up to 14 TeV. 
By the end of `Phase-1' data taking period, we will get data volume upto 300 ~\ifb, followed by a shutdown for the HL-LHC upgrade ending in 2026. The high luminosity data taking period with the upgraded LHC is called `Phase-2' and is expected to last from 2026 till 2038 and deliver integrated luminosity of of magnitude 3000 ~\ifb in the coming two decades, for both the CMS and ATLAS detectors. Brief description on upgrades has already been given in Section \ref{LHC_upgrade}. \\
Besides the planned upgrades for other sub-detectors of CMS, CSCs of its muon system will undergo several upgrades for HL-LHC. 

%\section{Main CSC upgrades for High Luminosity (HL)-LHC } \label{overview_upgrade}

Although CSCs are designed to give satisfactory performance throughout the HL-LHC operations, there are several phase 2 upgrades planned. 
%While the CSC modules are expected to provide satisfactory performance throughout the HL-LHC program,
\section{Electronics upgrades} \label{electronics_upgrade}
Increased CSC occupancy and as well as larger L1 trigger rates and latency requirements in Phase 2 will cause the pipeline storage of the front-end electronics (CFEBs and ALCTs) in certain rings to fill up. This will cause unacceptable dead time. 
Additionally, as a result of the higher CMS readout rates, the output bandwidth of the data acquisition boards 
(ODMBs and DDUs \footnote{Detector-Dependent Unit: It collects data, upon arrival of L1A, from all 15 DMBs in a CSC sector, corresponding to 30 or 60 degree $\phi$ slices of each endcap, and
sends the information through the global DAQ path.} ) will have to be expanded. 
%The increased occupancy in the chambers will necessitate the upgrade of the high voltage system.  upon arrival of L1A
Some CSC read-out electronic boards will need to be replaced in order to handle the more stringent trigger requirements. 
In general, the new boards will cope with higher L1 trigger rates and chamber occupancies by employing high-speed output optical links and faster processors. Details on the upgrades for off-chamber and on-chamber electronics is described in the Ref. \cite{CERN-LHCC-2017-012} and summarized in next sections. The main part of this electronics upgrade is happening during the Long Shutdown 2 (LS2) in 2019-2020. During the next shutdown the upgrade will be finalized by replacing the ODMBs and the FED system.

While designing the new electronics, an important aspect is taken into account. The radiation
hardness is relevant for the boards located on the chambers, which receive a significant dose, but
also for the TMBs and DMBs located (off-chamber) at the periphery of the CMS detector. Therefore, many
components such as optical transceivers, regulators, and EPROMs have been tested at several
irradiation facilities using neutron, proton, and mixed beams. In addition, the fully assembled boards
undergo irradiation tests to confirm that they can withstand the expected total integrated dose at the
HL-LHC. Moreover, the susceptibility of the electronics to single-event upsets (SEUs) \footnote{SEUs are changes of state caused by one single ionizing particle striking a sensitive node in microelectronic device.} is studied.
%SEUs are changes of state caused by one single ionizing particle striking a sensitive node in microelectronic device. 
The rate of these events as well as the resulting deadtime are monitored during the irradiation. 
%Similarly, the extended trigger latency requirement can be met with deeper (digital) buffers.
\subsection{Cathode front-end boards}
Current cathode front-end boards (CFEBs) on all CSC rings except ME1/1 \footnote{The ME1/1 chambers differ from all other chambers in that within a single station the electronics are mounted facing alternately toward and away from the IP. For the chambers centered at $\phi$ = 0, 10, 20 degree etc.; the electronics face towards, away, towards etc. the IP, respectively.} are based on analog technology of custom switched-capacitor array (SCA) chips that sample at 50 ns intervals to store the signal waveforms produced on the cathode strips when muon pass through them. %At the HL-LHC L1 latency of 12.5 ms and
%rate up to 500 kHz, the CFEBs in the 108 inner CSC chambers of ME2/1, ME3/1, and ME4/1 will overwrite capacitor cells in the SCAs causing large readout inefficiency.
%These capacitor arrays are capable of storing 96 charge measurements during the Level-1 trigger latency
%
%corresponding to a total of six events worth of data. 
Upon receiving a L1 signal for accepting event, the CFEBs digitize the charge measurements around the L1 time and the digital output is transferred to the data acquisition mother boards (DMBs). In parallel, fast trigger information is sent to the trigger mother boards (TMBs). These CFEBs are used on all CSCs \footnote{Except for those in the ME1/1 ring which was equipped with digital cathode front-end boards (DCFEBs) in 2013-2014 to improve rate capability.}. In
the ME2/1, ME3/1, and ME4/1 rings, which are also close to the beam, the HL-LHC trigger latency of 12.5 ms and the L1 trigger rate of 750 kHz will overwrite capacitor cells in the SCAs causing large readout inefficiency. Figure \ref{Fig:event_loss} shows the average event loss fraction for those stations as a function of the instantaneous luminosity. 
%These 
%curves are based on a statistical model that has been verified by measuring the loss rates in 
%bench tests that emulate expected background and L1 trigger rates. Neither a safety factor nor 
%uncertainties pertaining to the extrapolation to high luminosity are included. The figure shows 
%that the data loss rate would increase dramatically at the HL-LHC if the current CFEBs were maintained.

\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{Figures/event_loss.png}
\caption{CFEB (Phase-1) and DCFEB (Phase-2) event loss fractions for HL-LHC conditions 
in the inner CSC rings ME2/1, ME3/1, and ME4/1 as a function of instantaneous luminosity.
The design HL-LHC luminosity is marked by the dashed brown vertical
line. 
	}
\label{Fig:event_loss}
\end{center}
\end{figure}

The SCA overwriting problem can be resolved if the CFEBs in those rings are replaced with DCFEBs together with the replacement of the TMBs attached to those chambers by OTMBs. New DCFEBs are being designed with an option
that will allow for the remote programming of their FPGAs. DCFEBs currently existing over the ME1/1 chambers would be moved ot ME234/1 chambers which exposed to relatively lower radiation and hence new boards would be used for the ME1/1 chambers.  %will be installed in the ME1/1 chambers, thus mitigating any risks associated with
%their longevity, and the DCFEBs currently in the ME1/1 ring will be moved to chambers in the
%ME234/1 rings, which are subject to much lower levels of radiation. 
Top side of one of the DCFEBs produced for the ME1/1 upgrade having dimensions about $300 mm \times 175 mm$  with chips mounted on both sides can be seen in the Figure \ref{dcfeb_new}. 
Only minor modifications with respect to the LS1 DCFEB design are adopted to develope new DCFEBv2s which are being installed during LS2. 

\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{Figures/dcfeb_new.png}
\caption{One of the DCFEB used for ME1/1. 
        }
\label{dcfeb_new}
\end{center}
\end{figure}
...
%%%%%%%
\subsection{Anode local charged track boards}
The anode local charged track boards (ALCTs) are used to detect the deposits of charge on the CSC anode wires.
It forms the corresponding trigger primitives and pass the information to the DAQ upon receiving a L1 signal. 
These boards will
not have sufficient FPGA memory resources to keep all the raw hit information under high luminosity environment due to increased L1 trigger latency time. 
Moreover, the copper cable used in the
ALCTs to send the anode data to the TMBs does not provide enough output bandwidth for the boards in the rings closest to the beamline (ME1/1, ME2/1, ME3/1 and ME4/1) 
which will be insufficient for the expected HL-LHC data rates. Thus, all ALCT mezzanine
boards would be replaced as well, except those in ME1/1 and ME4/2 chambers that were
upgraded during LS1. In addition, the radiation hardness of the ALCT boards currently in
ME1/1 is a concern, so they will be moved to ME3/1 and new radiation-hard ALCTs will be
installed in ME1/1.

These problems are solved by replacing the mezzanine cards in the affected chambers by
new boards based on the design of the ALCT-S6 developed for the LS1 upgrade. The
Spartan-6 FPGAs have 9-12 times more memory than the currently used Virtex-E FPGAs allowing the pipeline storage to be made deep enough to satisfy the HL-LHC latency requirements. New boards developed for the LS1 upgrade as shown in the Figure \ref{alct_new}.

\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{Figures/alct_new.png}
\caption{The Spartan-6 ALCT mezzanine boards produced for the
ME1/1 upgrade which will be around
$104 mm \times 140 mm$ to accommodate the new optical components.
        }
\label{alct_new}
\end{center}
\end{figure}

\subsection{Trigger Mother Board}
Trigger Mother Board (TMB) builds cathode hit patterns and finds coincidences
with anode hit patterns to form local charged track primitives (LCTs). It sends up to
two LCTs to the MPC \footnote{Muon Port Card: It collects LCTs from each of up to nine TMBs in a trigger
sector, and sends them to the muon track finder system. There is one MPC per
peripheral crate.} (trigger path) and, upon receiving an L1A, to the DMB.
%The relocated DCFEBs and the new DCFEBv2s in the ME234/1 rings will transmit trigger primitive data to the peripheral crates via optical fibre links. 
The currently installed TMBs are designed to work with CFEBs, they have no optical receivers to accommodate the optical data link from the DCFEBv2s because DCFEBs  will transmit trigger primitive data to the peripheral crates via optical fibre links. 
To cope this situation, 108 OTMBv2 mezzanine boards and
OTMBv2 main boards are being built and installed during LS2 for the ME2/1, ME3/1, and ME4/1 rings. An OTMB mezzanine board produced for the ME1/1 upgrade is shown in the Figure \ref{otmb_upgrade}.

\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{Figures/otmb_upgrade.png}
\caption{Bottom side of one of the OTMB mezzanine boards produced for the ME1/1 upgrade. The OTMB mezzanine is about $190 mm \times 134 mm$ in size.
        }
\label{otmb_upgrade}
\end{center}
\end{figure}

During LS1,  OTMBs were designed and built for the ME1/1 during upgrade procedure. 
%The mezzanine
%boards employ a Reflex photonics Snap12 transceiver featuring 12 optical input receivers as
%shown in Fig. 4.12. 
The OTMB mezzanine board plugs into a VME OTMB baseboard which makes %The boards also
%have a relatively modern Xilinx Virtex-6 FPGA with built-in high speed serial transceivers.
use of an upgraded FPGA that provides the additional advantage of increased logic resources
to improve the trigger algorithms in high rate region.
%The Snap12 receivers are now obsolete so they will be replaced by another MTP12-
%type set of 12 receivers. 

%start tomorrow
\subsection{Data mother board}
The Data mother board (DMB) collects information from anode, 
cathode, and trigger and sends it to the DDU upon arrival of an L1A. It also controls the CFEBs
on a chamber.
Under HL-LHC running conditions, the 1.6 Gb/s output link in the DMBs and ODMBs will not be able
to handle the data rates in the inner CSC rings expected.
Two similar boards based on the ODMB design developed for the LS1 upgrade will increase
the output bandwidth of the affected stations: the 72 ODMBs in ME1/1 will be replaced by
ODMB7s that will connect to the seven DCFEBv2s in each ME1/1 CSC via the patch panel
shown in Section. \ref{csc_cms} Figure. \ref{Fig:csc_chamber_electronics} (right), and the 108 DMBs in ME234/1 will be replaced by ODMB5s that will
connect directly to the five DCFEB(v2)s in those CSCs as shown in the right of the same Figure. 
New optical fibers connecting the DCFEB(v2)s in ME234/1 with the corresponding ODMB5s will be laid out.

The designs of the ODMB5 and the ODMB7 differ in the front panel.
For the LS1 upgrade of ME1/1, the five CFEBs on each CSC module were replaced by seven
DCFEBs. Since there was not enough space on the front panel of the ODMB to fit seven HD50
connectors, a patch panel was used to reroute and fan out the signals coming from two 50-pin
Skewclear cables connected to the ODMB to the seven cables going to the DCFEBs. The front
panel of the ODMB7s will be largely the same as that in the current ODMBs, shown in Figure \ref{odmb}.
The design of the backplane connections, the clock and power distribution, and the FPGA and
PROM will be the same for the ODMB5 and ODMB7, and will be very similar to the design
of those parts in the existing ODMB. The main modification with respect to the ODMB 
corresponds to the upgrade of the FPGA from a Virtex-6 to the largest and highest speed grade Artix-7 model. 
The ODMB5 and ODMB7 will retain the upstream optical link with the FED board.

\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{Figures/odmb.png}
\caption{Top side of one of the ODMBs produced for the ME1/1 upgrade. The ODMB
is a 9U VME board, $400 mm \times 367 mm$ in size. The upgraded ODMB7 will be very similar
to the ODMB, while the ODMB5 will have a different front panel configuration with copper
connections to the five DCFEB(v2)s.
        }
\label{odmb}
\end{center}
\end{figure}
...
Front-End Driver (FED) crate constitutes the interface between the
front-end electronics and the global CMS data acquisition system.
For Phase 2, the 
CSC FED system will be upgraded to handle the 900 optical links at speeds up to 6.4 Gb/s replacing the current (O)DMB output links running at 1.6 Gb/s and 
delivering a total data rate of 598 Gb/s. 

The proposed architecture for the new CSC FED boards will likely be based on the ATCA
platform, the common CMS format for Phase 2. The trigger group is designing new generic
ATCA boards with different FPGA sizes. These boards would each have 80 to 104 optical links
with speeds up to 16 or 30 Gb/s, and also include a Zynq processor card and large memories
that could handle the remote programming of the DCFEB and ALCT FPGAs, exactly as needed
for the CSC DAQ. A total of twelve of these boards would be able to handle the CSC data rates
expected at the HL-LHC.
%
\subsection{Low and high voltage systems}
The low voltage (LV) and high voltage (HV) systems will also need to be upgraded to ensure the
reliable operation of the CSCs at the HL-LHC. The new electronics will consume more power,
which will require the redesign of the LV system with higher maximum currents and appropriate voltage levels. 
Higher background rates and the need for a more accurate monitoring of
CSC currents will also imply an incremental upgrade for the present HV system. %

Main low voltage upgrades are planned for new LVDBs. The LVDBs currently installed in the 108 CSCs in the ME234/1 rings provide the
CFEBs with voltage levels of 3.3 V, 5 V, and 6 V, while the DCFEBs require 3 V, 4 V,
and 5.4 V. New LVDB5s will be built and installed in the inner rings to provide the
appropriate voltage levels to the relocated DCFEBs. The LVDB5s will be similar to
the LVDB7s installed in ME1/1 during LS1 except that the former will power five
DCFEBs per CSC instead of seven. The Molex connectors will be replaced with
bolted lugs for greater robustness.

For high voltage set up, the present
Master boards will be replaced with thirty-eight Master boards of the new design each having 9 channels.
In the upgraded boards, each channel will have its own HV power source, capable of producing
voltages from 0 to 4000 V as output, with the maximum current of 2.5 mA (up from the present 1.5 mA). Master and Distribution boards for ME1/1 chambers will be installed in underground service cavern (USC).
The 24 output HV cables from Distribution boards will be routed to the rack where the current
ME1/1 HV system is located. That system will be replaced with a patch panel that mates output cables from Distribution boards with HV cables to ME1/1 chambers.
The technical scope of
the HV upgrade is summarized in Table \ref{hv_scope}.
...
\begin{center}
    ...
    &  non-ME1/1      & ME1/1         \\ \hline \hline
   HV master 9-channel boards &  38  &  2 \\
   HV distribution 36-channel boards &  -  &  12 \\
   Additional LV power supplies (commercial) & 1  &  1 \\
   Euro sub-racks &  8  &  3   \\ %\hline
   HV cables: distribution to CSC patch panels &  -  &  24 \\
   Patch panel sets &  -  &  1    \\ \hline \hline
    \end{tabular}
    \caption{CSC HV System upgrade scope.}
    \label{hv_scope}
\end{center}
\end{table}

%Finally, the L1 trigger rates will be too high for the current output bandwidth of the DMBs, so 
%these boards, together with the back-end modules that collect the CSC data and send it to the CMS DAQ, will need to be upgraded as well.

% UPGRADES OF CSC
\section{Activities during Long shutdown 2}
Figure \ref{Fig:dcfeb} shows a simplified overview of the CSC electronics upgrade. Given that the installation schedule in LS3 is driven by the replacement of various detectors elements in the barrel region, most of the CSC replacements that require chamber access will be performed ahead of that, during LS2.
All the CFEBs in the radially innermost CSCs will be equipped with slightly modified DCFEBs to cope with the HL-LHC requirements. 
The DCFEBs use 12-bit 20 MHz flash ADCs and powerful Virtex-6 FPGAs with large internal memory resources. The resulting digital pipeline can hold up to 700 events, which leads to negligible dead time at the HL-LHC.

\begin{figure}
\begin{center}
\includegraphics[width=0.8\linewidth]{Figures/dcfeb.png}
\caption{Simplified schematic of the current (left) and of the new (right) CSC electronics
for the inner rings formed by ME2/1, ME3/1 and ME4/1. The orange lines mark optical data
transfer, the blue lines represent copper cables..
        }
\label{Fig:dcfeb}
\end{center}
\end{figure}

%Radiation tests of the XCF128X PROMs employed in the current DCFEBs show that
%they may not withstand the radiation doses expected at the HL-LHC. Thus, the new DCFEBs
%are being designed with an option that employs the CERN-designed GBTx ASIC to allow for
%the remote programming of their FPGAs. These new boards will be installed in the ME1/1
%chambers, thus mitigating any risks associated with the longevity of their PROMs, and the
%DCFEBs currently in the ME1/1 ring will be moved to other chambers in the ME2/1, ME3/1,
%and ME4/1 rings subject to much lower levels of radiation.
%
%The DCFEBs send the trigger data via 3.2 Gb/s optical links, so the TMBs servicing the inner rings will be upgraded to optical TMBs (OTMBs). The low voltage distribution boards (LVDB) that supply power to the DCFEBs will be upgraded as well.
%The mezzanine boards for the ALCTs in both the inner and outer CSC rings (396 chambers in total), 
%not including the already upgraded ME1/1 and ME4/2 rings, will be upgraded with more powerful Spartan-6 FPGAs. 
%The inner ring boards will also incorporate optical links to cope with the HL-LHC data rates that will provide an output bandwidth of up to 6.4 Gb/s, ten
%times higher than previously. Finally, the boards to be installed in ME1/1 will be outfitted with
%the same remote FPGA programming option that the new DCFEBs will have, thus mitigating
%any risks associated with the aging of the PROMs in that ring.
%Similarly, the DMBs in the inner rings will be upgraded with new optical DMBs (ODMBs) to
%handle the HL-LHC data rates. These boards will have output bandwidths of up to 25.6 Gb/s,
%achieved with multiple 6.4 Gb/s links. The back-end electronics collecting all the CSC data will
%be upgraded accordingly.
%Altogether, a total of 1044 on-chamber electronics boards and 302 trigger and DAQ modules will be replaced in the CSC system. \\
%Table 1.6 summarizes the main new hardware components needed for this upgrade. \\
%\section{Activities during Long shutdown 2}

During ongoing long shutdown 2 (LS2), many of the above listed upgrades are in process for upcoming physics programs under high luminosity LHC.
CSC will undergo upgrade of readout system of ME1/1, ME2/1, ME3/1, and ME4/1. Total 180 chambers will be will be extracted from the CMS cavern and brought to the surface for their refurbishment with
upgraded DCFEBs and ALCT mezzanine boards, as well as LVDB5s in the case of ME234/1
chambers. Also for ME2/1, ME3/1 and ME4/1, LVDBs will be upgraded.\\
The DCFEB upgrade requires specific back-end electronics to be upgraded as well. The 36
crates located on the periphery of the YE2 and YE3 disks, servicing the 108 ME2/1, ME3/1 and
ME4/1 chambers will be equipped with the optical version of the TMBs (OTMB), as already
done in LS1 for ME1/1 chambers.
The detailed description of the upgrade plan of the involved electronics is given in the Table \ref{electronics_upgrades}.
\begin{table}[!htb]
 %\small
 \footnotesize
 %\scriptsize
\begin{center}
   ...
   Board   &  Number               &  Where          & When           &  Main reasons for upgrades  \\ \hline \hline
   DCFEBv2 &  540  &  ME12/1   &  LS2  &  Latency, rate, rad-hard requirements \\
   LVDB5 &  108  &  ME234/1   &  LS2  &  Power levels of DCFEBv2s \\
   ALCT-LX100 &  288  &  ME1/1, ME123/2   &  LS2/LS3  &  Latency, rate, rad-hard requirements \\
   ALCT-LX150T &  108  &  ME234/1   &  LS2  &  Latency and rate requirements \\ \hline
   OTMBv2 &  108  &  ME234/1   &  LS2  &  Receive optical link from DCFEBv2s \\
   ODMB7 &  72  &  ME1/1   &  LS3  &  Increased DAQ output bandwidth \\
   ODMB5 &  108  &  ME234/1   &  LS3  &  Increased DAQ output bandwidth \\
   HV master &  40  &  ME1234/1   &  LS3  &  Increased current due to higher occupancy \\  
   HV distribution &  12  &  ME1234/1   &  LS3  &  Increased current due to higher occupancy \\  
   FED &  14  &  USC   &  LS3  &  Increased data volume, number of links \\  \hline \hline
    \end{tabular}
    \caption{ Summary of the CSC electronics boards that will be upgraded during LS2 and LS3.
Boards above the horizontal line are placed on-chamber, while those below the line occur in
off-chamber crates. %Of the 540 new DCFEBv2s, 504 will be installed in the ME1/1 chambers,
%while the rest plus the DCFEBs currently in ME1/1 will be installed in ME2/1. 
The ALCTs in stations 2, 3, and 4 will be upgraded during LS2 while those in station 1 will be upgraded
in LS3. 
%The ALCTs in ME1/3 do not need upgrading because they will be replaced with the
%ALCT-S6 currently installed in ME1/1. 
	}
    \label{electronics_upgrades}
\end{center}
\end{table}
\par There is a surface facility in SX5 of CMS detector site which contains storage cradles and movable tables which
allow for in situ refurbishment. The facility is already equipped with test stands
which allows for board connectivity and functionality testing, as well as for long-term testing for early detection of potential electronics failure in the already refurbished chambers before reinstallation and final commissioning in CMS.

\subsection{CSC STEP testing}
System Test for Endcap Peripheral (STEP) crates and chamber electronics is a suite of tests designed to check the health of the setup and operation of CSCs before they are used in physics. It can detect connectivity problems, short-circuits, miscabling, faulty or out-of-specs components, excessive noise, crosstalk, etc.
A list of STEP tests which are performed on the chamber after its refurbishment is given in the Table \ref{step_tests}. However, a detailed description of full set of tests has been given in Ref. \cite{fast_tests_full}. % https://twiki.cern.ch/twiki/pub/CMS/CSCFastSX5/fast_tests_descrip.pdf

%\par A chamber after refurbishment is initially undergone through a set of tests done at low and high voltages. If a chamber passes these initial tests, it is put to other test stand for long term testing (LTT). Here the chamber is kept ON at low voltage and monitored for about 48 hours for voltage, current and temperatures of the DCFEBs and ALCTs whether they are consistent and within the allowed ranges. After LTT, chamber is examined for a subset of tests done before LTT monitoring called post-LTT tests. This completes the electronics testing and the chamber is then declared as ready for reinstallation.
...
\begin{center}
   ...
   id &  Description      & HV  and gass & remarks        \\ \hline \hline
   13 & AFEB Threshold and Analog Noise & -  & \\  \hline
   12 & AFEB Connectivity and Crosstalk & -  & not for ME1/1 \\  \hline
   11 & AFEB Counting Noise & required &  \\  \hline
   14 & ALCT-AFEB Time Delay & - &  \\  \hline
   15 & CFEB Pedestals and Noise & -  &  \\  \hline
   16 & CFEB Connectivity & - &  \\  \hline
   17 & CFEB Pulse Shape and Crosstalk & - &  \\  \hline
   17b & CFEB Gain & - &  \\  \hline
   19 & Comparator Thresholds and Analog Noise & - &  \\  \hline
   21 & Comparator Logic & - &  \\  \hline
   25 & ALCT Trigger & required & not underground \\  \hline
   27 &	High-Statistics Cosmics & required &  \\  \hline  \hline
    \end{tabular}
    \caption{List of STEP tests.}
    \label{step_tests}
\end{center}
\end{table}

The STEP software is based on the standard EmuLib CSC online software library, which in turn is built around XDAQ (the CMS online software suite) and also uses some modules of CMSSW (the CMS offline reconstruction program). The STEP measurement program is a XDAQ application with a web GUI. It uses the local DAQ for reading out the data and writing it to disk. The STEP analysis is a stand-alone program that can be run on the data files written by the STEP measurement program.
...
%bandwidth. Subsequently, a new pixel detector was installed in March 2017 Ref. \cite{Klein:2017nke}, equipped with one additional barrel  % The Phase-2 Upgrade of the CMS Tracker. Available online: https://cds.cern.ch/record/2272264 (accessed on 7 January 2019)
...
%interaction point and the outermost one is further away from it as it is shown in Figure  \ref{Fig:CMS_pixel_upgrade}. %(Figure 1)
%
%\begin{figure}
%\begin{center}
...
%In the figure ~\ref{Fig:hit_efficiency_upgrade}, clear improvments in the dynamic efficiency of 2017 (with pixel upgrades) with respect to the 2016 (without pixel upgrades) can be seen in Ref. ~\cite{Sonneveld:2018ntf}.
%
%\begin{figure}
%\begin{center}
...
%%%\end{figure}
%
%\subsection{Electromagnetic Calorimeter (ECAL) } \label{cms:ecal}
...
%A recent study on relative energy resolution of ECAL using 2017 data and MC has been performed using Z events decaying to a pair of electrons. The resolution plots for bremsstrahlung electrons ($R_{9}$>0.94 and $R_{9}$<0.94)\footnote{$R_{9}$ is ratio of the energy contained in a $3 \times 3$ ~crystal matrix and the supercluster energy} is separately shown for data and MC in the Figure \ref{Fig:CMS_ECAL_energy_resolution} ~\cite{Zghiche_2019}.  % https://iopscience.iop.org/article/10.1088/1742-6596/1162/1/012001/pdf 
...
%HCAL assembles with CMS system between the solenoid magnet and ECAL as can be seen in the Figure ~\ref{Fig:CMS_detector_design} at radial distance from 1.77 m to 2.95 m.
...
%HCAL provides pseudorapidity coverage upto $|\eta| < $ 3.0 beyond which, 11.2 m away from the interaction point, froward Hadron Calorimeter (HF) is employed on both sides of the detector providing additional coverage upto $|\eta| < $ 5.2. HF helps to detect particles from proton-proton collisions emerging at very small angles with respect the beam axis. \par
...
%When a particle passes through scintillator, optical fibers collect the emitted lights and pass it to readout box. 
...
%Identification of muons originating from the collision point is a great challenge. Having long lifetime (i.e. 2.19 \times $10^{-6}$ , which is even higher in relativitic regime), they tend to cross the whole detector without decaying. 
%Also, having higher mass (about 200 times the mass of electron) it tends to lose very little energy through process of bremsstrahlung. 
...
%SL is further composed of DTs in 4 layers \cite{Paolucci:927394}.  % http://cds.cern.ch/record/927394/files/CR2006_006.pdf
...
%a time resolution of about a nano-second. A schematic diagram of a RPC chamber taken from \cite{article_rpc} 
%%@article{article,
...
%Updated schematic diagram of CMS muon system with addition of GEM chambers can be seen in the Figure. ~\ref{Fig:updated_cms} (also in Ref. \cite{Radogna:2016esa, Costantini:2711362}) where the actual position of these chambers in muon system can be understood.
%
...
%Also, storing the uninteresting events produced in LHC collisions will require a lot of storage.
...
%Tier-0 facility is situated at CERN. It receives prompt data (RAW data) produced from LHC collisions as a primary backup and does the reconstruction of the events and store in the form of prompt-RECO data.
%It then exports the RAW data to to large computing centers at Tier-1 where the events are set to be reconstructed again with refined calibrations and at each Tier-1 site, a detailed reconstruction of RAW data is performed to get AOD(Analysis Object Data) version of data which is the most compact form of data. It also serves to distribute the AOD data to Tier-2 sites where the data can be used for end-user analysis specific reprocessing and simulations for all experiments.
%
%
%%
%%
