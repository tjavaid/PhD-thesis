...
The measurement of electron selection efficiency is made as a function of the probe electron $p_{T}$ and its SC $\eta$. For electrons falling in the ECAL gaps, it is measured separately. Figure \ref{fig:ele_sel_pt_turn_on} and \ref{fig:ele_sel_eta_turn_on} shows the $p_{T}$ and $\eta$ turn-on curves measured in data.
...
\begin{center}
	...
	\caption{Electron selection efficiencies vs $p_T$ (upper part of each plot) measured using the Tag-and-Probe technique, non-gap electrons (left) and gap electrons (right), together with the corresponding data/MC ratio (bottom part of each plot).}
...
Assessment of total uncertainty for measurement of the scale factors is taken as quadratic sum of statistical uncertainties (returned from the fit) and the aforementioned systematic uncertainties. Overall electron SFs and the associated total uncertainties can be seen in the Figure. \ref{fig:overall_elec_SF}. 
...
\vspace*{0.3cm}
\begin{center}
\includegraphics[width=1.0\textwidth]{Figures/egama_SF.png}
\end{center}
        \caption{Overall electron SFs (left) and associated uncertainties (right) in two dimensional display as function of $\pt$ and supercluster $\eta$.}
\label{fig:overall_elec_SF}
\end{figure}
