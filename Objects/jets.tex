...
In a high energy experiment e.g. proton-proton collision at LHC, jets are produced by quarks and/or gluons when they go through the process of hadronization and making a cone like structure in the detector like CMS. Hence, jets are physics objects which provide the experimental evidence of quarks and the gluons. 
%In a high energy experiment e.g. proton-proton collision at LHC, 
...
As these jets of particles propagate through the CMS detector, they leave signals in components such as the tracker and the electromagnetic and hadronic calorimeters. 
A typical diagram of the a jet can be seen in the Figure. \ref{fig:jet_cone} and in Ref. \cite{jets_at_cms} % http://cms.web.cern.ch/news/jets-cms-and-determination-their-energy-scale
Information from these signatures is exploited through some jet algorithms to fully reconstruct a ``jet''.

\begin{figure}[!htb]
\vspace*{0.3cm}
\begin{center}
\includegraphics[width=0.80\textwidth]{Figures/jet_cone.png}
\end{center}
        \caption{A schematic diagram of proton-proton collison and signature for a jet cone.}
\label{fig:jet_cone}
\end{figure}

The processes of Higgs production through fusion of vector bosons (VBF) and through other mechanisms normally differ as regards the jet kinematics.
...
For the analysis upon which the dissertation is built, jets are reconstructed through anti-$k_T$ clustering algorithm out of particle-flow candidates, with distance parameter $R = 0.4$, 
...
In addition, jets from Pile-Up are rejected using the PileUp jet ID criteria suggested by the JetMET POG~\cite{JetPUID2017} \footnote{In 2016, jet pileup ID is not applied (to be included in upcoming manuscript on the same subject).}. 
...
\par In the analysis, standard jet energy corrections are implemented to the reconstructed jets,
...
B-hadrons or b-jets have long life time $viz.$ about $1.5 ps$~ (those of c-hadrons have $1 ps$ or even less), and travel longer in the detector before they decay and subsequently leave the tracks of large impact parameter  depending upon the momentum.
These tracks are called displaced tracks and can help to reconstruct the secondary vertex (SV). 
A typical diagram of a heavy-flavour jet with a SV from the decay of b or c hadron resulting in charged-particle tracks is shown in the Figure. \ref{fig:bjet} and in Ref. \cite{Sirunyan:2017ezt}.
...
\vspace*{0.3cm}
\begin{center}
\includegraphics[width=0.80\textwidth]{Figures/bjet.png}
\end{center}
        \caption{A schematic diagram of a heavy flavor jet with a secondary vertex.}
\label{fig:bjet}
\end{figure}
...
Some validation studies are performed to probe agreement of data with simulated samples for jets. It is similar to what was done with electrons and muons.
...
%As it is shown, Jet Energy Resolution corrections are creating the ``horns" one can see in the MC for $\eta$ of about 2.8. Although not satisfactory, the disagreement of data from MC we see in this region is covered by the very large uncertainty. 
%After discussion with jet experts in the JetMET POG, it has been decided that this is the configuration we should use for this preliminary result. We also verified that it was not inducing significant migration of events in the signal MC between the categories.
